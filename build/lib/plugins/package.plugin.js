...
function packageIncomingMessage(message, routingKey) {
  ...
  const isPrePackaged = _.get(message, 'content.data') && _.get(message, 'content.dateIssued') && _.get(message, 'content.type', _.get(message, 'content._type')); // _type for backwards compat with 2.0.4
...
function outgoingMiddleware(next) {
  return (routingKey, message, options) => next(routingKey, packageOutgoingMessage(message, routingKey), options);
}
...
//# sourceMappingURL=package.plugin.js.map