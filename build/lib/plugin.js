...
function validatedPlugin(pluginExports = {}) {
  ...
  const invalidHooks = HOOKS.filter(hook => !(_.isUndefined(pluginExports[hook]) || _.isFunction(pluginExports[hook])));
  ...
  return {
    initalizePlugins: (plugins = []) => {
      ...
      initializedPlugins = plugins.map(pluginInitializer => {
        return validatedPlugin(pluginInitializer(pluginAPI));
      });
    },
